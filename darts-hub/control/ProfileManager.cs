...
namespace darts_hub.control
{
    ...
    public class ProfileManager
    {
        ...
        private void CreateDummyDownloadMaps()
        {
            ...
            dartsCallerDownloadUrl = dartsCallerDownloadMap.GetDownloadUrlByOs("0.1.4");
            ...
            dartsWledDownloadUrl = dartsWledDownloadMap.GetDownloadUrlByOs("0.1.9");
        ...
        private void CreateDummyAppsDownloadable()
        {
            ...
            if (!string.IsNullOrEmpty(dartsWledDownloadUrl))
            {
                var dartsWledArguments = new List<Argument> {
                        ...
                        new(name: "TO", type: "string", required: false, isMulti : true, nameHuman: "-TO / --takeout_effects", section: "WLED"),
                        new(name: "TOF", type: "string", required: false, isMulti : true, nameHuman: "-TOF / --takeout_effects_failure", section: "WLED")
                    };
                for (int i = 0; i <= 180; i++)
                {
        ...
        private void MigrateAppsDownloadable()
        {
            ...
            if (dartsWled != null)
            {
                if (dartsWledDownloadUrl != null)
                {
                    ...
                    {
                        dartsWled.Configuration.Arguments.Add(new(name: "TO", type: "string", required: false, isMulti : true, nameHuman: "-TO / --takeout_effects", section: "WLED"));
                    }
                    var takoutFailureDetection = dartsWled.Configuration.Arguments.Find(a => a.Name == "TOF");
                    if (takoutFailureDetection == null)
                    {
                        dartsWled.Configuration.Arguments.Add(new(name: "TOF", type: "string", required: false, isMulti : true, nameHuman: "-TOF / --takeout_effects_failure", section: "WLED"));
                    }
                }
            }
